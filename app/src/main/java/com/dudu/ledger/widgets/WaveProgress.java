...
import com.dudu.ledger.utils.MiscUtil;
...
public class WaveProgress extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mHintColor = typedArray.getColor(R.styleable.WaveProgress_hintColor, Color.WHITE);
        ...
        mCircleColor = typedArray.getColor(R.styleable.WaveProgress_circleColor, Color.WHITE);
    ...
    private void drawValueCircle(Canvas canvas) {
        ...
        circle.setAntiAlias(antiAlias);
        circle.setStrokeWidth(mCircleWidth);
        circle.setStyle(Paint.Style.FILL_AND_STROKE);
        circle.setStrokeCap(Paint.Cap.ROUND);
        circle.setColor(Color.parseColor("#40FFFFFF"));
        canvas.drawArc(mRectF, currentAngle, 360, false, circle);
        canvas.restore();
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        if (mPrePercent == 0.0f || Math.abs(mPercent - mPrePercent) >= 0.01f) {
            ...
            if (mPercent < 1 && mPercent != 0) mPercentValue = String.format("%.0f%%", mPercent * 100 + 1);
            mPrePercent = mPercent;
        }

        if (mPercent < 1) canvas.drawText(mPercentValue, mCenterPoint.x, y, mPercentPaint);
        else canvas.drawText("已完成", mCenterPoint.x, y, mPercentPaint);
    ...
    private void startDarkWaveAnimator() {
        ...
        mDarkWaveAnimator.addUpdateListener(animation -> {
